...
func genTsInterface(g *protogen.GeneratedFile, file *protogen.File, message *protogen.Message) (name string) {
	...
	if _, ok := interfaceCache[message.GoIdent.GoName]; !ok {
		for _, ff := range message.Fields {
			if ff.Desc.Kind() == protoreflect.MessageKind {
				...
				interfaceCache[ff.Enum.GoIdent.GoName] = getTsImportPath(file.Desc.Path())
	...
	interfaceCache[message.GoIdent.GoName] = getTsImportPath(file.Desc.Path())
	return
}
...
func genTsEnum(file *protogen.File, g *protogen.GeneratedFile) {
	for _, enum := range file.Enums {
		...
		g.P("}")
		g.P("")
		interfaceCache[enum.GoIdent.GoName] = getTsImportPath(file.Desc.Path())
	}
}
